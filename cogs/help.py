...
class CustomHelpCommand(commands.HelpCommand):
    ...
    def make_page_embed(
        self, commands, title="Eggolution Help", description=discord.Embed.Empty
    ):
        embed = discord.Embed(color=0x808080)
        embed.title = title
        embed.description = description
        embed.set_footer(
            text=f'Use "e!help command" for more info on a command.'
        )

        for command in commands:
            signature = 'e!' + command.qualified_name + " "
    ...
    def make_default_embed(
        self, cogs, title="Eggolution Categories", description=discord.Embed.Empty
    ):
        embed = discord.Embed(color=0x808080)
        embed.title = title
        embed.description = description
    ...
    async def send_bot_help(self, mapping):
        ...
        async def get_page(pidx, clear):
            ...
            embed = self.make_default_embed(
                cogs,
                title=f"Eggolution Command Categories (Page {pidx+1}/{len(pages)//6+1})",
                description=(
                    f"Use `e!help <command>` for more info on a command.\n"
                    f"Use `e!help <category>` for more info on a category."
                ),
            )
    ...
    async def send_command_help(self, command):
        ...
        embed.title = 'e!' + command.qualified_name
